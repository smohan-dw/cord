...
pub use cord_braid_twist_runtime::RuntimeGenesisConfig as BraidTwistRuntimeGenesisConfig;
...
use cord_braid_twist_runtime::SessionKeys as BraidTwistSessionKeys;
...
pub use cord_braid_base_runtime_constants::currency::UNITS;
// as BRAID_UNITS;
...
pub const ENDOWMENT: Balance = 500_000_000 * UNITS;
...
fn braid_base_session_keys(
	...
	BraidBaseSessionKeys { babe, grandpa, authority_discovery }
}
...
fn braid_plus_session_keys(
	...
	BraidPlusSessionKeys { babe, grandpa, authority_discovery }
}

fn braid_twist_session_keys(
	babe: BabeId,
	grandpa: GrandpaId,
	authority_discovery: AuthorityDiscoveryId,
) -> BraidTwistSessionKeys {
	BraidTwistSessionKeys { babe, grandpa, authority_discovery }
}
...
fn cord_braid_twist_custom_config_genesis(config: ChainParams) -> serde_json::Value {
	let initial_network_members: Vec<AccountId> =
		config.network_members.iter().map(array_bytes::hex_n_into_unchecked).collect();
	...
	let initial_sudo_key: AccountId = array_bytes::hex_n_into_unchecked(&config.sudo_key);
	cord_braid_twist_custom_genesis(
		initial_network_members,
		initial_well_known_nodes,
		initial_authorities,
		initial_sudo_key,
...
pub fn cord_custom_config(config: ChainParams) -> Result<CordChainSpec, String> {
	...
	if runtime_type == "base" {
		...
		.with_name(&chain_name)
		.with_id("braid-twist-custom")
		.with_chain_type(chain_type)
		.with_genesis_config_patch(cord_braid_twist_custom_config_genesis(config.clone()))
		.with_telemetry_endpoints(
			TelemetryEndpoints::new(vec![(CORD_TELEMETRY_URL.to_string(), 0)])
				.expect("Cord telemetry url is valid; qed"),
		)
...
fn cord_braid_base_custom_genesis(
	...
	// const ENDOWMENT: Balance = ENDOWMENT;
...
fn cord_braid_twist_custom_genesis(
	initial_network_members: Vec<AccountId>,
	initial_well_known_nodes: Vec<(NodeId, AccountId)>,
	initial_authorities: Vec<(AccountId, BabeId, GrandpaId, AuthorityDiscoveryId)>,
	root_key: AccountId,
	...
	serde_json::json!( {
		...
		"babe":  {
			"epochConfig": Some(cord_braid_twist_runtime::BABE_GENESIS_EPOCH_CONFIG),
		},
		"sudo": { "key": Some(root_key) },
	})
}
